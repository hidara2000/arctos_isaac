...
class PickPlaceController(BaseController):
    ...
    def __init__(
        self,
        name: str,
        cspace_controller: BaseController = None,
        ...
        if self._h1 is None:
            self._h1 = 0.1 / get_stage_units()
        self._h0 = None
        self._events_dt = events_dt
        if self._events_dt is None:
            self._events_dt = [
                0.05,  # 0 move end_effector above the cube center
                0.002,  # 1 Lower end_effector down to encircle the target cube
                0.1,  # 2 Wait for Robot's inertia to settle.
                0.001,  # 3 close grip.
                0.005,  # 4 Move end_effector up
                0.002,  # 5 Move end_effector towords goal
                0.004,  # 6 Move end_effector vertically toward goal height
                0.01,  # 7 loosen the grip
                0.004,  # 8 Move end_effector vertically up
                0.008,  # 9 Return home
            ]
        else:
        ...
        if not cspace_controller:
            self._cspace_controller = RMPFlowController(
                name=name + "_cspace_controller", robot_articulation=robot_articulation
            )
        else:
            self.cspace_controller = cspace_controller
    ...
    def forward(
        ...
        if self._event == 2:
            ...
            target_joint_positions.joint_positions[-2] = 0
            target_joint_positions.joint_positions[-1] = 0
        else:
            ...
            if end_effector_orientation is None or self._event >= 3:
        ...
        self.log += f"{self._event}, {', '.join(map(str, target_joint_positions.joint_positions))}\n"

        if self._event >= 3 and self._event < 7:
            gripper_pos = self._gripper.forward(action="close")
            # TODO fix this at a lower level to keep centered as mimic joints are buggy
            grip_mean = np.mean(gripper_pos.joint_positions[-2:]) - self.dt_squeeze
            target_joint_positions.joint_positions[-2] = grip_mean
            target_joint_positions.joint_positions[-1] = grip_mean

        self.dt_squeeze = 0.000009
        return target_joint_positions

    def _get_interpolated_xy(self, target_x, target_y, current_x, current_y):
        alpha = self._get_alpha()
        xy_target = (1 - alpha) * np.array([current_x, current_y]) + alpha * np.array(
            [target_x, target_y]
        )
        return xy_target
    ...
    def save_logs(self):
        Path("./dara_log_gripper.csv").write_text(self.log)
